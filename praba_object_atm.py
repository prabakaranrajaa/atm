...
def thanks():
    print("Thank you for banking with PRABA BANKS")

# welcoming the user and assigning values to some variables


def main():
    print("WELCOME TO PRABA BANK")
...
# elif the user selects "2", it shows "how much" and then shows the total balance
            ...
            case "3":
                user_name = input("where do to wand send name? ")
                for student in students:
                    if student["name"] == user_name:
                        ...
                        if amount > balance:
                            ...
                            print("your balance after transfer to ",
                                  student["name"], "is ", balance)
...
#                print(students[user_index])
                ...
                with open("student.csv", "w") as file:
                    writer = csv.DictWriter(
                        file, fieldnames=["name", "password", "balance"])
                    for student in students:
                        writer.writerow(
                            {"name": student["name"], "password": student["password"], "balance": student["balance"]})
                break
